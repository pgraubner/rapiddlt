...
use crate::{read_typed, TIterator, WithOffset, TSearchable, grepit::GrepIterator};
...
impl<'bytes> TSearchable<'bytes> for WrapTestStruct<'bytes> {
    fn marker() -> &'static [u8] {
        &[0x1,0x2,0x3,0x4]
    }
...
#[cfg(test)]
mod tests {
    use crate::{searchit::{SearchIterator, RevSearchIterator}, readit::ReadIterator};
    ...
    fn readit_num_matches() {
        test_num_matches::<ReadIterator<WrapTestStruct>>(TestKind::Forward);
    }

    #[test]
    fn readit_robust_offset() {
        test_robust_offset::<ReadIterator<WrapTestStruct>>(TestKind::Forward);
    }

    #[test]
    fn readit_skipped_simple() {
        test_skipped_simple::<ReadIterator<WrapTestStruct>>();
    }

    #[test]
    fn readit_wrap_values() {
        test_wrap_values::<ReadIterator<WrapTestStruct>>();
    }
